...
function highlightTags($tags, $searchTerm)
{
    ...
    return str_replace($searchTerm, "<span class = 'bg-yellow-200'>{$searchTerm} </span>", $tags_str);
}

function getAvgSalary($listings)
{
    $total = 0;
    ...
    return '$' . number_format($total / count($listings), 2, '.', ',');
}
?>
...
<body class="bg-gray-100">
    ...
    <div class="bg-green-100 rounded-lg shadow-md p-6 my-6">
        <h2 class="text-2xl font-semibold mb-4">Average Salary: <?php echo getAvgSalary($listings); ?></h2>
    </div>
